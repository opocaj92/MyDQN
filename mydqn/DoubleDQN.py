...
''' This class represent the double deep Q-Network itself. It is initialized with
many parameters used to create properly the experience replay memory and the two
networks used by the training algorithm. The copy_params() methods simply updates
the parameters of the target_network with those of the q_network. The train() method
is the one that properly train the whole DQN. The difference with a standard DQN is
the use of the best actions predicted by the Q-network instead of the target network
and their Q values in computing the update. '''
class DoubleDQN:
        ...
        for episode in range(num_episodes):
            ...
            for t in itertools.count():
                ...
                if t % update_every == 0:
                    ...
                    q_next_values = self.q_network.predict(next_observations_batch)
                    best_actions = np.argmax(q_next_values, axis = 1)
                    double_q_next_values = self.target_network.predict(next_observations_batch)
                    target_values = rewards_batch + np.invert(done_batch).astype(np.float32) * discount_factor * double_q_next_values[np.arange(batch_size), best_actions]
